...
from ConvTran.AbsolutePositionalEncoding_ import tAPE, AbsolutePositionalEncoding, LearnablePositionalEncoding
from ConvTran.Attention import Attention, Attention_Rel_Scl, Attention_Rel_Vec
...
class Transformer2(nn.Module):
    def __init__(self, input_shape, hidden_size, length,ffh):
        super().__init__()
        # Parameters Initialization -----------------------------------------------
        channel_size, seq_len = input_shape, length
        emb_size = hidden_size
        num_heads = 1
        dim_ff = hidden_size*ffh
        ...
        self.Fix_Position = tAPE(emb_size, dropout=0.01, max_len=seq_len)


        self.LayerNorm1 = nn.LayerNorm(emb_size, eps=1e-5)
        self.LayerNorm2 = nn.LayerNorm(emb_size, eps=1e-5)

        self.attention_layer = Attention_Rel_Scl(emb_size, num_heads, seq_len, 0.01)
